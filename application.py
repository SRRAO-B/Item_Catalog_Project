...
# DBSession() instance (session) establishes all
# conversations with the database
...
def newCategory():
    '''New category. Handles GET and form POSTs
    '''
...
def editCategory(category_id):
    '''Edit category. Handles GETS and form POSTs
    '''
    if "username" not in login_session:
        return redirect("/login")
    ...
    try:
        ...
        vendor = getUserInfo(category.user_id)
    except NoResultFound:
        flash("Category not found")
        return redirect(url_for("categories"))

    if vendor.id != login_session['user_id']:
        flash("You're not the vendor who owns this category. Not authorized!")
...
def deleteCategory(category_id):
    '''Deleting category. Handles GETS and form POSTs
    '''
    if "username" not in login_session:
        return redirect("/login")
...
def items(category_id):
    ''' Viewing all items in a category
    '''
    categories = session.query(Category).all()
    try:
        category = session.query(Category).filter_by(id=category_id).one()
...
def viewItem(category_id, item_id):
    ...
    try:
        ...
        return redirect(url_for("categories"))
    return render_template("viewitem.html", category=category,
                           item=item, login_session=login_session)


@app.route("/category/<int:category_id>/new/", methods=["GET", "POST"])
def newItem(category_id):
    '''Creating a new item in a category
    '''
    if "username" not in login_session:
        return redirect("/login")
...
def editItem(category_id, item_id):
    '''Editing an item. Handles GETS and form POSTs
    '''
    if "username" not in login_session:
        return redirect("/login")
    ...
    if vendor.id != login_session['user_id']:
        flash("You do not own these category items. Not authorized!")
        return redirect(url_for("items", category_id=category.id))
...
def deleteItem(category_id, item_id):
    '''Deleting an item. Handles GETS andform POSTs
    '''
    if "username" not in login_session:
        return redirect("/login")
    try:
    ...
    if vendor.id != login_session['user_id']:
        flash("You do not owns these category items.Not authorized!")
        return redirect(url_for("items", category_id=category.id))
...
def gconnect():
    ...
    CLIENT_ID = json.loads(open("client_secrets.json", "r").
                           read())['web']['client_id']
    APPLICATION_NAME = "Ebikes Catalog"
...
def gdisconnect():
    '''Disconnects a user from site using Google OAuth2 Returns: Boolean.
        True if successfully logged user out and revoked token
    '''
...
def fbdisconnect():
    ...
    Returns: Boolean.  True if successfully logged user out and revoked token
    '''
...
if __name__ == "__main__":
    app.secret_key = "c8MEAApjExANRI7gtQsIDUbA"
    app.debug = True
    app.run()
